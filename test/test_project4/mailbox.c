...
#define DATA_LENGTH (2*(MAX_MBOX_LENGTH))
...
void recv_thread(void *arg)
{
    ...
    for (;;)
    {
        ...
        int remain_size = DATA_LENGTH * 2 - bytes[other_id[0] - 'a'] - bytes[other_id[1] - 'a'];
        ...
        sys_move_cursor(1, position);
        printf("[%c-recv] from %c: %ld \t from %c: %ld, pos: %d (%lx), remain: %d, len: %d    \n",
            id, other_id[0], bytes[other_id[0] - 'a'],
            other_id[1], bytes[other_id[1] - 'a'], position, &position, remain_size, len);
...
void send_thread(void *arg)
{
    ...
    for (;;)
    {
        len = rand() % (MAX_MBOX_LENGTH / 4 - 2) + 1;
        target = rand() % 2;
        if (bytes[target] >= DATA_LENGTH) target = 1 - target;
        if (len >= DATA_LENGTH - bytes[target]) len = DATA_LENGTH - bytes[target];
